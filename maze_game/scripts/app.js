require(["levels"], function (Levels) {
    ...
    aMaze.currentName = document.getElementById("name"),
    aMaze.currentDescription = document.getElementById("description"),
    ...
    //#region Player
    function Player(x, y, direction,level) {
        this.x = x;
        this.y = y;
        this.direction = direction;
        this.weapon =  new Bitmap(level.playerWeapon.path, level.playerWeapon.width, level.playerWeapon.height);
        this.paces = 0;
    }
        ...
        if (this.y > map.size || this.y < 0 || this.x > map.size || this.x < 0) {
            if (!aMaze.mazeCompleted) {
                ...
                if (Levels.length == nextLevel) {
                    ...
                    if (confirm(aMaze.currentMessage + "\r\n Click OK to move on to the next level or CANCEL to play this level over again.")) {
                        aMaze.currentLevel++;
                    }
    ...
    function Map(level) {
        ...
        this.skybox = new Bitmap(level.skybox.path, level.skybox.width, level.skybox.height);
        this.wallTexture = new Bitmap(level.wallTexture.path, level.wallTexture.width, level.wallTexture.height);
    ...
    Camera.prototype.drawColumn = function (column, ray, angle, map) {
        ...
        for (var s = ray.length - 1; s >= 0; s--) {
            var step = ray[s];
                    var rainDrops = Math.pow(Math.random(), 10) * s;
    ...
    aMaze.init = function (level) {
        ...
        aMaze.currentMessage = level.message;
        aMaze.currentLevelDisplay.innerHTML = "Level: " + level.id;
        aMaze.currentName.innerHTML = "Name: " + level.name;
        aMaze.currentDescription.innerHTML = "Description: " + level.description;
        aMaze.mazeCompleted = false;
        aMaze.player = new Player(level.startingPoint.x,level.startingPoint.y,level.startingDirection,level);

        aMaze.map = new Map(level);
        aMaze.controls = new Controls();
        aMaze.camera = new Camera(aMaze.display, aMaze.MOBILE ? 160 : 320, 0.8);
